...
pub fn get_levels() -> Vec<Level> {
    vec![
        ...
        Level {
            ...
            test_cases: vec![(vec![123], vec![0]), (vec![999], vec![0])],
        },
        Level {
            id: "05_Inc&Dec".to_string(),
            name: "Inc & Dec".to_string(),
            description: "Read input A (RDI). Output A+1 and A-1 (Stream Output via OUT)."
                .to_string(),
            test_cases: vec![(vec![10], vec![11, 9]), (vec![0], vec![1, -1])],
        },
        Level {
            id: "06_Unconditional".to_string(),
            name: "Unconditional".to_string(),
            description: "Output 1 infinitely (Stream). (Test checks for first 5 outputs of 1)."
                .to_string(),
            test_cases: vec![(vec![0], vec![1, 1, 1, 1, 1])],
        },
        Level {
            id: "07_ZeroFlag".to_string(),
            name: "Zero Flag".to_string(),
            description: "Read A (RDI). If 0, return 1. Else return 0 (RAX).".to_string(),
            test_cases: vec![(vec![0], vec![1]), (vec![5], vec![0]), (vec![-1], vec![0])],
        },
        Level {
            id: "08_SignFlag".to_string(),
            name: "Sign Flag".to_string(),
            description: "Read A (RDI). If A < 0, return 1. Else return 0 (RAX).".to_string(),
            test_cases: vec![(vec![-5], vec![1]), (vec![0], vec![0]), (vec![10], vec![0])],
        },
        Level {
